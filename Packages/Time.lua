...
--> CUSTOM
function Time.GetFormattedPerkTime(time)
	local day = Time.getDay(time) - 1
	local hour = Time.getHour(time)
	local minutes = Time.getFormattedMinute(time)
	local seconds = Time.getFormattedSecond(time - minutes * 60)

	if day > 0 then
		return day .. " day" .. (day > 1 and "s" or "")
	end
...
function Time.FormatSent(time)
	local day = Time.getDay(time) - 1
	local hour = Time.getHour(time)
	local minutes = Time.getMinute(time)
	local seconds = Time.getSecond(time - minutes * 60)

	if day > 0 then
		return string.format("%i day%s ago", day, day > 1 and "s" or "")
	end

	if hour > 0 then
		return string.format("%i hour%s ago", hour, hour > 1 and "s" or "")
	end

	if minutes > 0 then
		return string.format("%i minute%s ago", minutes, minutes > 1 and "s" or "")
	end

	if seconds > 0 then
		return string.format("%i second%s ago", seconds, seconds > 1 and "s" or "")
	end
end
