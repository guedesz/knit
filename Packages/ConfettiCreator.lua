...
local types = Knit:GetAsset("ConfettiTypes")

local deleteConfettiAfter = 2 --Increasing the time will decrease performance
...
function CreateOne(origin, colors, minSize, maxSize)
	...
	Debris:AddItem(confetti, deleteConfettiAfter)
...
function CreateConfetti(
	...
	maxUpwardVelocity = maxUpwardVelocity or 1000
	minHorizontalSpeed = minHorizontalSpeed or 0
	maxHorizontalSpeed = maxHorizontalSpeed or 15
	minDownwardAcceleration = minDownwardAcceleration or 50
	maxDownwardAcceleration = maxDownwardAcceleration or 100
	minAngularSpeed = minAngularSpeed or 0
	maxAngularSpeed = maxAngularSpeed or 10
	...
	for i = 1, count do
		task.spawn(function()
			...
			MoveOne(confetti, upwardVelocity, horizontalVelocity, downwardAcceleration, angularVelocity)
		end)
	end
end
