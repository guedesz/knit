...
function KnitServer:GetAsset(assetName : string)
	
	local serverAssets = ServerStorage:WaitForChild("assets")
	local clientAssets = ReplicatedStorage:WaitForChild("assets")

	local asset = serverAssets:FindFirstChild(assetName, true)

	if not asset then
		asset = clientAssets:FindFirstChild(assetName, true)
	end

	if not asset then
		return warn("No asset found with given name: ", assetName)
	end
...
function KnitServer:GetMetaData(moduleName: string)
	
	local secondTest = ReplicatedStorage:WaitForChild("src"):WaitForChild("Data"):FindFirstChild(moduleName, true)

	if secondTest and secondTest:IsA('ModuleScript') then
...
function KnitServer:GetModule(moduleName : string)
	local firstTest = ServerStorage:WaitForChild("src"):WaitForChild('ServerObjects'):FindFirstChild(moduleName, true)
	if firstTest and firstTest:IsA('ModuleScript') then
		return require(firstTest)
	end

	local test = ReplicatedStorage.packages:FindFirstChild(moduleName, true)
	if test and test:IsA('ModuleScript') then
		return require(test)
	end

	local secondTest = ReplicatedStorage:WaitForChild("assets"):WaitForChild("Modules"):FindFirstChild(moduleName, true)

	if secondTest and secondTest:IsA('ModuleScript') then
		return require(secondTest)
	end

	local thirdTest = ServerStorage:WaitForChild("assets"):WaitForChild("Modules"):FindFirstChild(moduleName, true)

	if thirdTest and thirdTest:IsA('ModuleScript') then
		return require(thirdTest)
	end

	return print("Error getting module", moduleName)
end
