...
local function GetLength(t: Table): Table
...
function DictionarySelectRandom(dictionary)
	local keys = {}
...
function GetRandomItemsWithoutDuplicates(dictionary, amount)
	local numItems = TableUtil.GetLength(dictionary)
    
    if numItems < amount then
        return dictionary  -- Not enough items available, return the count
    end

    local selectedItems = {}
    
    while #selectedItems < amount do
        local index = math.random(1, numItems)
        local item = dictionary[index]
...
TableUtil.GetRandomItemsWithoutDuplicates = GetRandomItemsWithoutDuplicates
TableUtil.GetLength = GetLength
TableUtil.Copy = Copy
TableUtil.Sync = Sync
...
TableUtil.ArraySelectRandom = ArraySelectRandom
TableUtil.DictionarySelectRandom = DictionarySelectRandom
